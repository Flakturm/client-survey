...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, language, '.$this->tables['users'].'.company')
						  ->from($this->tables['users'])
						  ->join($this->tables['company'], $this->tables['company'].'.company_id = '.$this->tables['users'].'.company', 'inner')
						  ->join($this->tables['geo_zone'], $this->tables['company'].'.geo_zone_id = '.$this->tables['geo_zone'].'.geo_zone_id', 'inner')
						  ->join($this->tables['languages'], $this->tables['geo_zone'].'.language_id = '.$this->tables['languages'].'.language_id', 'inner')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get();
	...
	public function companies()
	{
		...
		return $this->db->get($this->tables['company'])->result_array();
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', username, email, id, last_login, language, '.$this->tables['users'].'.company')
						  ->from($this->tables['users'])
						  ->join($this->tables['company'], $this->tables['company'].'.company_id = '.$this->tables['users'].'.company', 'inner')
						  ->join($this->tables['geo_zone'], $this->tables['company'].'.geo_zone_id = '.$this->tables['geo_zone'].'.geo_zone_id', 'inner')
						  ->join($this->tables['languages'], $this->tables['geo_zone'].'.language_id = '.$this->tables['languages'].'.language_id', 'inner')
	...
	**/
	...
	public function company($id) {
		return $this->db->get_where($this->tables['company'], array('company_id' => $id))->row();
	}
